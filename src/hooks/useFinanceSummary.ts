...
export function useFinanceSummary() {
  ...
  return useMemo(() => {
    ...
    const monthTransactions = transactions.filter((item) => monthKey(new Date(item.date)) === currentMonthKey);
    const yearTransactions = transactions.filter((item) => new Date(item.date).getFullYear() === currentYear);
    ...
    const byCategory = monthTransactions
      ...
      .reduce<Record<TransactionCategory, number>>((acc, item) => {
    ...
    const budgetStatus = budgets.map((budget) => ({
      ...
      percent: budget.limit > 0 ? Math.min(1, (byCategory[budget.category] ?? 0) / budget.limit) : 0
    }));
    ...
    const cardUsage = creditCards.map((card) => {
      const spent = transactions
        .filter((item) => item.type === "expense" && (item.cardId === card.id || item.sourceBank === card.bank))
        .reduce((sum, item) => sum + item.amount, 0);
      return { ...card, spent, available: Math.max(0, card.totalLimit - spent), percent: card.totalLimit > 0 ? spent / card.totalLimit : 0 };
    });
    const forecast = Array.from({ length: 3 }).map((_, offset) => {
      const date = new Date(now.getFullYear(), now.getMonth() + offset + 1, 1);
      const recurringIncome = recurringEntries.filter((item) => item.active && item.type === "income").reduce((sum, item) => sum + item.amount, 0);
      const recurringExpenses = recurringEntries.filter((item) => item.active && item.type === "expense").reduce((sum, item) => sum + item.amount, 0);
      const installments = transactions
        .filter((item) => item.installmentTotal && item.installmentCurrent && item.installmentCurrent + offset < item.installmentTotal)
        .reduce((sum, item) => sum + item.amount, 0);
      return {
        month: monthKey(date),
    ...
    const alerts: FinancialAlert[] = [
      ...
      ...goals
        .filter((goal) => goal.monthlyAmount && goal.targetAmount > 0 && goal.savedAmount / goal.targetAmount < 0.35)
        .slice(0, 2)
        .map((goal) => ({
          id: `goal-${goal.id}`,
...
function sumByType(items: { type: "income" | "expense"; amount: number }[], type: "income" | "expense") {
...
function buildMonthlyEvolution(transactions: { date: string; type: "income" | "expense"; amount: number }[], months: number) {
  const now = new Date();
  return Array.from({ length: months }).map((_, index) => {
    ...
    return { month: key, income: sumByType(items, "income"), expenses: sumByType(items, "expense") };
  });
}

function buildAnnualEvolution(transactions: { date: string; type: "income" | "expense"; amount: number }[], years: number) {
  const now = new Date();
