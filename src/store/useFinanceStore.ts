...
const demoGoals: Goal[] = [
  ...
  {
    id: 'g-3',
    title: 'Viagem para ver a familia',
    targetAmount: 1200,
    savedAmount: 310,
    color: '#7DB7D9',
    emoji: '+',
...
export const useFinanceStore = create<FinanceState>()(
  persist(
    set => ({
      ...
      addHabit: habit => set(state => ({ habits: [{ ...habit, id: uid('h') }, ...state.habits] })),
      ...
      resetHabitsIfNewDay: () =>
        set(state => {
          ...
          if (state.lastHabitResetDate === todayKey) return {};

          return {
            lastHabitResetDate: todayKey,
            habits: state.habits.map(habit => ({ ...habit, doneToday: false })),
          };
        }),
      addDiaryEntry: entry =>
        set(state => ({ diary: [{ ...entry, id: uid('d') }, ...state.diary] })),
      updateDiaryEntry: (id, data) =>
        set(state => ({
          diary: state.diary.map(entry => (entry.id === id ? { ...entry, ...data } : entry)),
        })),
      ...
      addMotivationalQuote: quote =>
        set(state => ({ motivationalQuotes: [...state.motivationalQuotes, quote] })),
      updateMotivationalQuote: (index, quote) =>
        set(state => ({
          motivationalQuotes: state.motivationalQuotes.map((item, itemIndex) =>
            itemIndex === index ? quote : item,
          ),
        })),
      deleteMotivationalQuote: index =>
        set(state => ({
          motivationalQuotes: state.motivationalQuotes.filter(
      ...
      undoInvoiceImport: importId =>
        set(state => ({
          ...
          transactions: state.transactions.filter(
            transaction => transaction.invoiceImportId !== importId,
          ),
        })),
      addAccount: account =>
        set(state => ({ accounts: [{ ...account, id: uid('acc') }, ...state.accounts] })),
      ...
      addCreditCard: card =>
        set(state => ({ creditCards: [{ ...card, id: uid('card') }, ...state.creditCards] })),
      ...
      removeCreditCard: id =>
        set(state => ({ creditCards: state.creditCards.filter(card => card.id !== id) })),
      addRecurringEntry: entry =>
        set(state => ({
          recurringEntries: [{ ...entry, id: uid('rec') }, ...state.recurringEntries],
        })),
      updateRecurringEntry: (id, data) =>
        set(state => ({
          recurringEntries: state.recurringEntries.map(entry =>
            entry.id === id ? { ...entry, ...data } : entry,
          ),
        })),
      removeRecurringEntry: id =>
        set(state => ({
          recurringEntries: state.recurringEntries.filter(entry => entry.id !== id),
        })),
      resetDemoData: () =>
        set({
          salary: 2800,
    ...
    }),
    {
      ...
      merge: (persisted, current) => ({
        ...
        _hasHydrated: false,
        categories: (persisted as Partial<FinanceState>)?.categories?.length
          ? (persisted as Partial<FinanceState>).categories!
          : demoCategories,
        accounts: (persisted as Partial<FinanceState>)?.accounts?.length
          ? (persisted as Partial<FinanceState>).accounts!
          : demoAccounts,
        creditCards: (persisted as Partial<FinanceState>)?.creditCards?.length
          ? (persisted as Partial<FinanceState>).creditCards!
          : demoCreditCards,
