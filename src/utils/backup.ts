...
export async function importBackupFromJson(jsonText: string) {
  ...
  if (Array.isArray(data.invoiceImports))
    useFinanceStore.setState({ invoiceImports: data.invoiceImports });
  ...
  if (Array.isArray(data.recurringEntries))
    useFinanceStore.setState({ recurringEntries: data.recurringEntries });
}
...
export async function exportFinancialReport(format: 'csv' | 'excel' | 'pdf') {
  ...
  const content =
    format === 'csv'
      ? toCsv(transactions)
      : format === 'excel'
        ? toExcelHtml(transactions)
        : toSimplePdf(transactions);
...
function toSimplePdf(transactions: Transaction[]) {
  const lines = [
    ...
    ...transactions
      .slice(0, 80)
      .map(
        item => `${item.date.slice(0, 10)}  ${formatAmount(item.amount, item.type)}  ${item.title}`,
      ),
  ];
  const stream = lines
    .map((line, index) => `BT /F1 10 Tf 40 ${780 - index * 14} Td (${escapePdf(line)}) Tj ET`)
    .join('\n');
  const objects = [
...
function encodeBase64(value: string) {
  ...
  for (let index = 0; index < value.length; index += 3) {
    ...
    output +=
      ...
      (hasB ? alphabet[(triple >> 6) & 63] : '=') +
      (hasC ? alphabet[triple & 63] : '=');
  }
  return output;
}
