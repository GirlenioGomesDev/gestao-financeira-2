import { InvoiceSource, ParsedInvoice, ParsedInvoiceTransaction, TransactionType } from "@/types/finance";
...
export function parseInvoiceText(text: string, source: InvoiceSource, fileName?: string): ParsedInvoice {
  ...
  const transactions = dedupeParsedRows(rows.map((row) => parseTransactionLine(row, bank)).filter(Boolean) as ParsedInvoiceTransaction[]);
  ...
  const totalAmount = explicitTotal ?? transactions.filter((item) => item.type === "expense").reduce((sum, item) => sum + item.amount, 0);
  const warnings: string[] = [];

  if (source === "pdf" && transactions.length === 0) {
    warnings.push("Nao encontrei transacoes neste PDF. Se ele for imagem escaneada, use foto/OCR ou CSV.");
  }

  if (explicitTotal && Math.abs(explicitTotal - totalAmount) > 1 && transactions.length > 0) {
    warnings.push("O total informado na fatura e diferente da soma dos itens lidos. Confira antes de importar.");
  }
...
function parseCsvRows(text: string) {
  ...
  const delimiter = [",", ";", "\t"].find((item) => firstLines.split(item).length > firstLines.split("\n").length * 1.6);
  if (!delimiter) return [];
...
function rowsFromCsv(rows: string[][]) {
  ...
  const dataRows = header.some((cell) => /(data|descricao|descrição|valor|historico|histórico)/.test(cell)) ? rows.slice(1) : rows;
  const dateIndex = findColumn(header, ["data", "date"]);
  const descriptionIndex = findColumn(header, ["descricao", "descrição", "historico", "histórico", "estabelecimento", "memo", "lancamento"]);
  const amountIndex = findColumn(header, ["valor", "amount", "debito", "débito", "credito", "crédito"]);
...
function extractAmount(line: string) {
  const matches = [...line.matchAll(/(?:R\$\s*)?([+-]?\s?\d{1,3}(?:\.\d{3})*,\d{2}|[+-]?\s?\d{1,6}[.,]\d{2})(?:\s*(CR|CREDITO|CRÉDITO|DB|DEBITO|DÉBITO))?/gi)];
  ...
  const isCredit = signed.startsWith("+") || /(credito|crédito|\bcr\b|estorno|pagamento recebido|cashback)/i.test(raw) || /(credito|crédito|estorno|cashback)/.test(lower);
...
function extractInvoiceTotal(text: string) {
  ...
  for (const line of lines) {
    const lower = normalize(line);
    if (!/(total da fatura|valor total|total\s*:|valor da fatura|saldo total)/.test(lower)) continue;
