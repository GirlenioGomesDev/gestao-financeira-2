...
export default function ImportHistoryScreen() {
  ...
  return (
    <PaperScreen>
      ...
      {imports.map(item => (
        <View key={item.id} className="mb-4 rounded-paper border border-line bg-surface p-4">
          ...
          <AppText className="mt-1 text-sm text-muted">
            {item.transactionCount} transacoes • {formatCurrency(item.totalAmount)} •{' '}
          ...
          <PrimaryButton
            ...
            onPress={() =>
              Alert.alert(
                'Desfazer importacao?',
                'Todas as transacoes desta fatura serao removidas.',
                [
                  { text: 'Cancelar', style: 'cancel' },
                  {
                    text: 'Desfazer',
                    style: 'destructive',
                    onPress: () => undoInvoiceImport(item.id),
                  },
                ],
              )
            }
      ...
      {imports.length === 0 ? (
        <AppText className="text-muted">Nenhuma fatura importada ainda.</AppText>
      ) : null}
    </PaperScreen>
  );
}
