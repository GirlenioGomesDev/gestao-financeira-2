...
export default function InvoiceReviewScreen() {
  ...
  function findDuplicate(item: ParsedInvoiceTransaction) {
    return existing.find(transaction => {
      ...
      const similarDescription =
        left.length >= 6 &&
        right.length >= 6 &&
        (left.includes(right.slice(0, 8)) || right.includes(left.slice(0, 8)));
  ...
  return (
    <PaperScreen>
      <SectionHeader
        title="Fatura Lida"
        subtitle={`${parsed.bank} - ${parsed.referenceMonth} - ${activeItems.length} transacoes`}
      />
      ...
      <View className="mb-4 rounded-paper border border-primary bg-primary/10 p-4">
        ...
        <AppText className="mt-2 text-sm text-muted">
          {items.length} gastos encontrados - {activeItems.length} ativos -{' '}
          {items.length - activeItems.length} ignorados
        </AppText>
        <AppText className="mt-1 text-sm text-muted">
          Soma dos gastos listados: {formatCurrency(listedTotal)}
        </AppText>
        {parsed.warnings?.map(warning => (
          <AppText key={warning} className="mt-1 text-sm text-coral">
            {warning}
          </AppText>
        ))}
      </View>
      ...
      {items.map((item, index) => (
        <View
          key={item.id}
          className={`mb-3 rounded-paper border border-line p-4 ${item.ignored ? 'bg-line opacity-60' : 'bg-surface'}`}
        >
          <View className="mb-2 flex-row justify-between">
            ...
            <Pressable onPress={() => updateItem(item.id, { ignored: !item.ignored })}>
              <AppText className={item.ignored ? 'text-primaryDark' : 'text-coral'}>
                {item.ignored ? 'Desfazer' : 'Ignorar'}
              </AppText>
            </Pressable>
          </View>
          <EditableField
            ...
            type="text"
            displayStyle="card"
            onSave={value => updateItem(item.id, { description: String(value) })}
          />
          <EditableField
            value={item.amount}
            type="currency"
            displayStyle="card"
            onSave={value => updateItem(item.id, { amount: Number(value) })}
          />
          <EditableField
            value={item.type}
            type="select"
            ...
            displayStyle="card"
            onSave={value =>
              updateItem(item.id, { type: value === 'income' ? 'income' : 'expense' })
            }
          />
          <EditableField
            value={item.category}
            ...
            onSave={value =>
              updateItem(item.id, {
                category: String(value) as ParsedInvoiceTransaction['category'],
              })
            }
          />
          <EditableField
            ...
            type="date"
            displayStyle="card"
            onSave={value => updateItem(item.id, { date: new Date(String(value)).toISOString() })}
          />
          {findDuplicate(item) ? (
            <AppText className="mt-2 text-xs text-coral">Possivel duplicata ja registrada.</AppText>
          ) : null}
      ...
      <View className="rounded-paper border border-line bg-surface p-4">
        <AppText className="mb-1 font-body text-lg">
