  ...
  return (
    <PaperScreen>
      ...
      <View className="mb-4 rounded-paper border border-line bg-surface p-4">
        ...
        <EditableField value={userName} type="text" displayStyle="title" onSave={(value) => setUserName(String(value))} />
        <EditableField
      ...
      <EditableField value={salary} type="currency" label="Salario mensal" displayStyle="card" onSave={(value) => setSalary(Number(value))} />
      <View className="mt-3 flex-row gap-3">
        <MoneyCard label="Saldo do mes" value={balance} tone={balance >= 0 ? "good" : "bad"} />
        <MoneyCard label="Saldo atual" value={currentBalance} tone={currentBalance >= 0 ? "good" : "bad"} />
      </View>
      ...
      <View className="my-5 rounded-paper border border-line bg-surface p-4">
        <View className="mb-3 flex-row items-center justify-between">
          ...
          <AppText className="font-body text-base text-primaryDark">{Math.round(salaryUsage * 100)}%</AppText>
        </View>
        <ProgressBar value={salaryUsage} color={salaryUsage > 0.8 ? "#E96C5F" : "#2F8F6B"} />
        <AppText className="mt-3 text-sm text-muted">
          {salaryUsage > 0.8 ? "Atenção: este mes pede escolhas mais leves." : "Voce ainda tem folego para planejar com calma."}
        </AppText>
      </View>
        ...
        {cardUsage.map((card) => (
          <View key={card.id} className="mb-3 rounded-paper bg-paper p-3">
            <View className="mb-2 flex-row items-center justify-between">
              ...
              <AppText className="text-sm text-muted">{formatCurrency(card.available)} livre</AppText>
            </View>
            <ProgressBar value={card.percent} color={card.percent > 0.85 ? "#E96C5F" : card.color} />
            <AppText className="mt-2 text-xs text-muted">Fecha dia {card.closingDay} - vence dia {card.dueDay}</AppText>
          </View>
        ))}
      </View>
      ...
      <View className="mb-5 rounded-paper border border-line bg-surface p-4">
        ...
        {forecast.map((item) => (
          <View key={item.month} className="mb-2 flex-row items-center justify-between rounded-paper bg-paper p-3">
            <AppText>{item.month}</AppText>
            <AppText className={item.balance >= 0 ? "text-primaryDark" : "text-coral"}>{formatCurrency(item.balance)}</AppText>
          </View>
        ))}
      </View>
      ...
      {mainGoal ? (
        <View className="my-5 rounded-paper border border-line bg-surface p-4">
          ...
          <EditableField value={mainGoal.title} type="text" displayStyle="title" onSave={(value) => updateGoal(mainGoal.id, { title: String(value) })} />
          <ProgressBar value={mainGoal.savedAmount / mainGoal.targetAmount} color={mainGoal.color} />
          <EditableField value={mainGoal.targetAmount} type="currency" displayStyle="subtitle" onSave={(value) => updateGoal(mainGoal.id, { targetAmount: Number(value) })} />
          <AppText className="mt-2 text-sm text-muted">{formatCurrency(mainGoal.savedAmount)} guardados</AppText>
        </View>
      ) : null}
      ...
      <View className="mb-5">
        ...
        {budgetStatus.slice(0, 4).map((budget) => (
          <View key={budget.category} className="mb-3 rounded-paper border border-line bg-surface p-3">
            <View className="mb-2 flex-row items-center justify-between">
              <View className="flex-row items-center">
                <Ionicons name="ellipse" size={8} color={budget.percent > 0.9 ? "#E96C5F" : "#2F8F6B"} />
      ...
      <View className="mb-4 flex-row items-center justify-between">
        ...
        <PrimaryButton label="Adicionar" icon="add" className="min-h-10 px-3" onPress={() => setModalVisible(true)} />
      </View>
      {transactions.slice(0, 4).map((transaction) => (
        <Pressable key={transaction.id} className="mb-3 flex-row items-center rounded-paper border border-line bg-surface p-3">
